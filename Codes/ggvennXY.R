...
my.ggvenn <- function(data, data.plus, columns = NULL,
                      show_elements = FALSE,
                      show_percentage_X = TRUE, show_percentage_Y=F,
                      digits = 1,
                      ...
                      auto_scale = FALSE)
{
  ...
  venn <- prepare_venn_data(data, columns, show_elements, show_percentage_X, digits,
  ...
  venn.plus <- prepare_venn_data(data.plus, columns, show_elements, show_percentage_Y, digits,
                                 label_sep, count_column, show_outside, auto_scale)
  if(length(data) ==4){
    hjust.add <- c(-1, 0, 0, 0.5)
    vjust.add <- - c(1, -1, -1, 1)
  }else{
    hjust.add <- 0
    vjust.add <- -c(-1, -1, 1)
  ...
  if (nrow(venn$labels) > 0) {
    g <- g +
      ggtext::geom_richtext(data = venn$labels, # + c(-1, 0, 0, 0.5), - c(1, -1, -1, 1)
                aes(x = x, y = y, label = text, hjust = hjust + hjust.add , vjust = vjust + vjust.add),
                color = set_name_color,
                size = set_name_size, fill=NA, label.colour=NA)
  }
  ...
  if (nrow(venn.plus$texts) > 0) {
    g <- g +
      geom_text(data = venn.plus$texts,
                aes(x = x, y = y, label = paste0("[", text, "]"), hjust = hjust, vjust = vjust+vjust.plus),
...
# "#0073C2FF", "#EFC000FF", "#868686FF", "#CD534CFF" (with red)
